...
## 2026-09-10 PCR revise 长期冻结（内部方向，未实现）

- revise 阶段一次选择性能、正确性、sim-real 一致性和返修说服力综合最优的可实施方案；只有影响核心架构、训练成立或论文结论的高风险不确定性才专门验证。低概率低影响猜测采用合理工程默认，不以小测试或横向扩审计/消融/场景/参数拖延开训；必要实现正确性检查仍须完成，且不得编造结果。
- 地图最终冻结为 sim/PCR/real 同语义即时 canonical local map；real-only obstacle-map memory 关闭，PCR learned-w risk memory 保留。无 dropout 小测试或再分支讨论；相机实测 offset 等未知不伪造、也不新增测试要求。它不是代码、训练、实机或审稿回复结果。

## 2026-09-10 最新优先声明：Avoid 1-D 基线（内部设计，非审稿回复结论）

- 作者已批准下一批设计方向：Avoid 改为真正的 1-D 横移专家，并以 `v_drive_nom` 取代历史第 14 槽 `forced_forward_speed`；训练、PCR 与实机应最终调用同一 canonical builder。
- 本轮未实现、未训练、未验证该设计，也未核实或改写任何历史训练/实机事实。因此它不能作为对 R1-16 的实验回答、不能宣称优于历史 3-DoF Avoid，亦不能进入投稿回复的完成项。
- 当前已完成的仅是来源取证与设计边界；real-only actor obstacle-map memory 已冻结为关闭，PCR risk memory 独立保留；地图其余实现细节未落地。相机 origin/offset 实测值仍未知，不得伪造或额外要求测试。详见 [内部基线](docs/specs/AVOID_1D_BASELINE_20260910.md)。
...
- 该诊断启动曾因日志未重定向、旧类延迟加载、未激活既有 Conda 的 PATH，以及误传 `skill` 先后失败，均未产生有效 rollout；使用已有 `isaac_gym` 环境并修正 metadata 参数后 `EXIT_CODE=0`。未安装依赖、未训练、未改旧 root 或 GPU1 Avoid；这不是 corrected-code Isaac smoke，也不放行正式训练。
