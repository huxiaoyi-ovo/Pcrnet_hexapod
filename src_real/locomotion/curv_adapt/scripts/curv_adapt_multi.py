...
class CurvAdapt_multi:
    def __init__(self,robot_config_yaml,device,env_nums,
        ...
        #swing 
        if self.cfg.high_body:
            self.swing_init_point[...,0]=0.16 # x
            self.swing_init_point[...,2]=0.08
            self.swing_init_point[[2,5],0]=0.17
            self.swing_init_point[[0,4],1]=0.06
            self.swing_init_point[[1,3],1]=-0.06            
        else:
            self.swing_init_point[...,0]=0.19 # x
            self.swing_init_point[...,2]=0.07
            self.swing_init_point[[2,5],0]=0.2
            self.swing_init_point[[0,4],1]=0.065
            self.swing_init_point[[1,3],1]=-0.065
        ...
        self.q_init=torch.zeros(env_nums,6,4,dtype=torch.float32,device=device)
        self.B_e_init=torch.ones(env_nums,6,4,dtype=torch.float32,device=device)
        ...
        self.torques_cur=joint_torques_cur # env_nums*6*3 float32 tensor
        ...
        self.torques_cur_flat=self.torques_cur.view(env_nums*6,3)
        ...
        for i in range(3): 
            ...
            self.q_init_flat[:,i]=q_init[i]
        #set corner positions
        self.q_init[:,[1,3],0]=-0.2
        self.q_init[:,[0,4],0]=0.2
        # self.kin.InverseKin1(self.B_e_des[:,0:3],self.q_init[:,0:3])
        self.kin.ForwardKin(self.q_init_flat,self.B_e_init.view(-1,4))
        self._GetFootAngle(self.q_init_flat)
        self.q_des.copy_(self.q_init)
        self.B_e_des.copy_(self.B_e_init)
    ...
    def ProcessCommand(self,command:torch.Tensor)->torch.Tensor: #command: env_nums*[set_init,vx,vy,vz,omega]
        #update current position of foot end in leg base frame
        self.kin.ForwardKin(self.q_cur_flat,self.B_e_cur_flat)
        # set_init_mask=(command[:,0].to(torch.bool))&(self.set_init_done<=100)
        # # set_init_mask=(command[:,0].to(torch.bool))
        # # print("set_init_mask:",set_init_mask)

        # if set_init_mask.any():
        #     # self.swing_reach_high[set_init_mask]=False
        #     self.swing_reach_point[set_init_mask]=False
        #     self.stance_group_index[set_init_mask]=0
        #     self.gaits[set_init_mask]=0
        #     set_init_index=torch.where(set_init_mask)[0]
        #     self.gaits[set_init_index.unsqueeze(-1),self.A_group_index]=1
        #     self.q_des[set_init_mask,...]=self.q_init[set_init_mask,...]
        #     self.B_e_des[set_init_mask,...]=self.B_e_init[set_init_mask,...]
        #     self.adhesions[set_init_mask,...]=0

        #     set_adhesion_index=torch.where(set_init_mask&(self.set_init_done>=80))[0]
        #     if set_adhesion_index.numel()>0:
        #         self.adhesions[set_adhesion_index.unsqueeze(-1),self.A_group_index]=1

        #     add_count_mask=(torch.norm((self.B_e_des-self.B_e_cur),p=2,dim=2)<0.02).all(dim=1)
        #     # print("norm\n",(torch.norm((self.B_e_des-self.B_e_cur),p=2,dim=2)))
        #     # print("B_e_des\n",self.B_e_des)
        #     # print("B_e_cur\n",self.B_e_cur)
        #     # print("add_count_mask",add_count_mask) # env_nums*6 6 legs within range,set true
        #     self.set_init_done[add_count_mask]+=1
        
        # # process_command_mask
        # process_command_mask=(command[:,0].to(torch.bool))&(self.set_init_done>100)
        # moving_env_mask=command[:,1].to(torch.bool)|command[:,2].to(torch.bool)|command[:,4].to(torch.bool)
        # if process_command_mask.any():
        #     if moving_env_mask.any():
        ...
        if self.cfg.high_body:
            ...
            self.swing_init_point[[2,5],0]=0.193
            self.swing_init_point[[0,4],1]=0.07
            self.swing_init_point[[1,3],1]=-0.07
        ...
        if reset_mask.any():
            ...
            self.B_e_des[reset_index,...]=self.B_e_init[reset_index,...]
    ...
    def GaitPlanning(self,command:torch.Tensor):
        # print(">>>>>>>>>>>>>GaitPlanning<<<<<<<<<<<<")
        ...
        adsorb_release_mask=self._AdsorbReleaseDetection() # env_nums*6 bool tensor
    ...
    def CalJointPoses(self):
        ...
        damp_inv_jac_env=self.kin.DampInvJac(self.q_cur_flat) 
        pos_err=(self.B_e_des_flat[...,0:3]-self.B_e_cur_flat[...,0:3]).unsqueeze(-1) # (env_nums*6)*3*1
        self.q_des_flat[:,0:3]=self.q_cur_flat+40*((damp_inv_jac_env@pos_err).squeeze(-1))*self.dt
        # self._GetFootAngle(self.q_des_flat)

        # self.q_des_flat[:,0:3]=self.q_cur_flat
    ...
    def _TargetTransInterp(self,command:torch.Tensor):#command: env_nums*[set_init,vx,vy,vz,omega]
        for i in range(2):
            sign = 1 if i == 0 else -1.8
            x_env = command[:,1] * self.dt * sign
    ...
    def _CalB_e_des(self,command)->torch.Tensor:
        ...
        # v_cmd += torch.abs(command[:,4])*0.4
        # v_cmd =torch.clip(v_cmd,max=1.2)

        # v_cmd=torch.norm(command[:,1:3],dim=1)*0.5 #num_envs
        # v_cmd += torch.abs(command[:,4])*0.2
        # v_cmd = torch.clip(v_cmd,max=1.0)
        v_cmd=torch.norm(command[:,1:3],dim=1)*1.2 #num_envs
        ...
        # print("next_B_e_des:\n",next_B_e_des)
        # # swing set z
        #choose siwng & not reach high legs
        # add_z_mask=(~self.gaits & (~self.swing_reach_high).unsqueeze(1))
        # next_B_e_des[...,2][add_z_mask]+=self.vec_z[add_z_mask]*self.dt
        # # for those envs, any of use add_z_mask and reach high, set swing_reach_high
        # set_swing_reach_high_mask=((next_B_e_des[...,2]>=0.03)&add_z_mask).any(dim=1) #finish swing_reach_high
        # self.swing_reach_high[set_swing_reach_high_mask]=True
            
        # sub_z_mask=(~self.gaits & self.swing_reach_high.unsqueeze(1))
        # next_B_e_des[...,2][sub_z_mask]-=self.vec_z[sub_z_mask]*self.dt
        # print("swing_reach_high:\n",self.swing_reach_high)
    ...
    def _FeasiCheck(self,B_e_des:torch.Tensor)->torch.Tensor:
        ...
        xy_feasi=(xy_len>=0.14) & (xy_len<=0.32)
    ...
    def _ContactDetection(self)->torch.Tensor:
        ...
        if self.cfg.high_body:
            return (self.B_e_des[...,2]<-0.18)&(self.swing_reach_point)
        else:
            return (self.B_e_des[...,2]<-0.14)&(self.swing_reach_point)
    
    def _AdsorbReleaseDetection(self)->torch.Tensor:
        # print("suction_forces:\n",self.suction_forces)
        stance_release_done=self.gaits&(self.suction_forces<self.cfg.max_suck_force*0.1)
        swing_adsorb_done=~self.gaits&(self.suction_forces>self.cfg.max_suck_force*0.85)
        # print("self.suction_forces:",self.suction_forces)
