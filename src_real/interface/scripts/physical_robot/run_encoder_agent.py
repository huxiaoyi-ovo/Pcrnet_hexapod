from BC_learning.Agent_utils import ActorCriticEncoder, StorageMemory
import torch
import os
import rospy
...
def ProcessCommand(msg:joy_command):
    ...
    command[1]=command[1]* 0.4 * 2.0
    command[2]=command[2]* 0.4 * 2.0
    command[3]=command[3]* 1.5 * 0.25
    #拼接观测，要乘上观测值对应的系数
    obs = torch.cat([
        quat,
        omega * 0.25,
        acc,
        (q_cur-q_default).reshape(-1),
        q_dot_cur.reshape(-1) * 0.05,
        q_torq.reshape(-1) * 0.1,
        command[1:]
    ])
    # obs = torch.cat([
    #     last_actions*0.5,
    ...
    with torch.inference_mode():
        action=agent.act_interence(obs,storage)
        # print("storage msg mask\n",storage.mask)
        last_actions = action
        #还原到关节期望角度
        action = action.reshape(6,3)*0.5+q_default
        # action = action*0.2+q_default
        estimate_pub.publish(q_des_msg_list[0])
        for i in range(6):
            if msg.set_init==0:
            ...
            q_des_pub_list[i].publish(q_des_msg_list[i])
    #用于记录encoder的信息
    obs_vgf_estimate=agent.obs_vgf_estimator(obs)
    # print("obs_vgf_estimate=\n",obs_vgf_estimate)
    estimate_vgf_msg.data.extend(obs_vgf_estimate)
    estimate_pub.publish(estimate_vgf_msg)
    estimate_vgf_msg.data=[]
...
def UpdateRootState(root_state_msgs:Float64MultiArray):
    ...
    quat=torch.tensor(root_state_msgs.data[0:4],dtype=torch.float32,device=device)
    omega=torch.tensor(root_state_msgs.data[4:7],dtype=torch.float32,device=device)
    acc=torch.tensor(root_state_msgs.data[7:10],dtype=torch.float32,device=device)
...
if __name__ == '__main__':
    ...
    # agent=BC_Agent()
    agent = ActorCriticEncoder().to(device)
    agent.eval()
    storage=StorageMemory(device=device)

    model_path='/home/nvidia/agents/'+args.agent #为了方便复制粘贴,放到主目录下方
    ...
    q_des=torch.zeros(6,3,dtype=torch.float32,device=device)
    q_cur=torch.zeros(6,3,dtype=torch.float32,device=device)
    q_dot_cur=torch.zeros(6,3,dtype=torch.float32,device=device)
    q_torq=torch.zeros(6,3,dtype=torch.float32,device=device)
    #填充关节默认角度
    for i in range(6):
        if i==0 or i==4:
            thigh=0.5
    ...
    #IMU观测
    acc = torch.zeros(3,dtype=torch.float32,device=device)
    omega = torch.zeros(3,dtype=torch.float32,device=device)
    quat = torch.zeros(4,dtype=torch.float32,device=device)
    #上一次动作
    last_actions=torch.zeros(18,dtype=torch.float32,device=device)
    #结束标志位
    ...
    estimate_pub=rospy.Publisher('/estimate_vgf',Float64MultiArray,queue_size=10)
    q_des_pub_list=[]
    q_cur_sub_list=[]
    q_des_msg_list=[] #Float64MultiArray()
    estimate_vgf_msg=Float64MultiArray()
