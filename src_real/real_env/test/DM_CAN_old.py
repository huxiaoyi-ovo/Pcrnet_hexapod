...
class Motor:
    def __init__(self, MotorType, SlaveID, MasterID):
        ...
        self.ISenable = ENable_Type.Disable
        self.mode = Control_Type.MIT
...
class MotorControl:
    ...
    def controlMIT(self, DM_Motor, kp: float, kd: float, q: float, dq: float, tau: float):
        ...
        # print("controlMIT",bytes(self.send_data_frame.T).hex())
    ...
    def control_Pos_Vel(self, Motor, P_desired: float, V_desired: float):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        print("control_Pos_Vel",bytes(self.send_data_frame.T).hex())
        # self.recv()  # receive the data from serial port
    ...
    def control_Vel(self, Motor, Vel_desired):
        ...
        self.serial_.write(bytes(self.send_data_frame.T))
        # print("control_Vel",bytes(self.send_data_frame.T).hex())
        # self.recv()  # receive the data from serial port
    ...
    def control_pos_force(self,Motor,Pos_des:float,Vel_des,i_des):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        # print("control pose force",bytes(self.send_data_frame.T).hex())
        # self.recv()  # receive the data from serial port
    ...
    def enable(self, Motor):
        ...
        Motor.ISenable=ENable_Type.Enable
        # print("in here")
    ...
    def disable(self, Motor):
        ...
        Motor.ISenable=ENable_Type.Disable
    ...
    def recv(self,Motor):
        data_buf = np.array([Motor.SlaveID, 0x00, 0xcc, 0x00], np.uint8)
        send=self.send_data_frame.copy()
        send[13] = 0xFF
        send[14] = 0x07
        send[21:25] = data_buf
        # self.serial_.write(bytes(self.send_data_frame.T))
        data_recv = self.serial_.read_all()
        print(bytes(data_recv).hex())
    ...
    def control_cmd(self, Motor, cmd: np.uint8):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        print("control_cmd",bytes(self.send_data_frame.T).hex())
        # self.recv()  # receive the data from serial port
    ...
    def read_motor_param(self, Motor, RID):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        print("read param",bytes(self.send_data_frame.T).hex())

        self.recv(Motor)  # receive the data from serial port
    ...
    def write_motor_param(self, Motor, RID, data):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        print("write param",bytes(self.send_data_frame.T).hex())
        time.sleep(0.01)
        self.recv(Motor)  # receive the data from serial port

        if Motor.mode==Control_Type.POS_VEL:
            self.send_data_frame[14] = 0x01
        elif Motor.mode==Control_Type.VEL:
            self.send_data_frame[14] = 0x02
        elif Motor.mode==Control_Type.MIT:
            self.send_data_frame[14] = 0x00
        # print("write param",bytes(self.send_data_frame.T).hex())
    ...
    def save_motor_param(self, Motor):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        # print("save_motor_param",bytes(self.send_data_frame.T).hex())
        # self.recv()
