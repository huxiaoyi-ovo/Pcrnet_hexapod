...
- Table II: current Risk-only source is eval-time; do not describe it as trained from scratch in the paper.
- Table II: Delta y_r (all) can be non-zero while Delta y_r@C_avoid is near zero because C_avoid is a narrow utility-preferred avoid window.
- Table II: Risk-only Delta y_w@C_avoid should be 0.000; Learned-w Delta y_w@C_avoid should be non-zero.
