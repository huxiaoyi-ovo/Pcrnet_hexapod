...
- 执行记录：初次直接解释器启动因未激活现有 Conda 而缺 Ninja，随后发现旧 checkpoint 要求 `skill=moe`；两次均未进入 rollout。改用已有 `isaac_gym` 环境且仅修正该 metadata 参数后成功，未安装依赖、未训练、未修改旧 root 或 GPU1 Avoid。产物：`/home/dell/RL_hexapod_gym_revision_geometry_20260908/outputs/actor_xy_old29ee_293c518_retry4/`；script SHA `4d49fa36…`，low-level 两路径 hash 已核为相同。
