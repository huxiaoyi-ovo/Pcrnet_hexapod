...
- [ ] [P0] 完成 Learned-w 推理期因果消融：先落地 `No-Delta-y_w` eval-only 开关，同一 learned-w checkpoint 下仅强制 `delta_y_w_used=0`，保留 `delta_y_w_raw`、`y_eff/cmd` 重算和 protocol 标记；后续再按需要扩展禁用 `Delta y_r`、`risk_memory`，生成 inference ablation 表。
